...
class RabbitMqListener {
    ...
    async listeners(): Promise<void> {
        ...
        this.updateUserWithActiveProfileIdListener();
        this.checkPhoneNumberDisponibilityListener();
    }

    private async updateUserWithActiveProfileIdListener(): Promise<void> {
        const channel = await this.rabbitmq.createChannel(RabbitMqQueues.UPDATE_USER_WITH_PROFILE_DATA);
        channel.consume(RabbitMqQueues.CREATE_SESSION, async (message: ConsumeMessage | null) => {
            ...
            let response: IValidationTokenData = await this.updateUserWithActiveProfileId(content);
            this.rabbitQueueResponse(channel, message, response);
            channel.ack(message);
        });
    }

    private async checkPhoneNumberDisponibilityListener(): Promise<void> {
        const channel = await this.rabbitmq.createChannel(RabbitMqQueues.CHECK_PHONE_NUMBER_DISPONIBILITY);
        channel.consume(RabbitMqQueues.CHECK_PHONE_NUMBER_DISPONIBILITY, async (message: ConsumeMessage | null) => {
            if (!message) return;
            const content: IUpdateUserWithPhoneNumberAndActiveProfileId = JSON.parse(message.content.toString()).data;
            let response: boolean = await this.checkPhoneNumberDisponibility(content);
            this.rabbitQueueResponse(channel, message, response);
            channel.ack(message);
        });
    }
